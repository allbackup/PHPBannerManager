...
include (DIR_SERVER_ADMIN."header.php");
...
if($HTTP_GET_VARS['format'] == "image" || $HTTP_GET_VARS['format']=="swf")
{
	...
	if($HTTP_GET_VARS['banner_id'] && $HTTP_POST_VARS['image_insert'])
	{
		...
		refresh("banners.php?user_id=".$HTTP_GET_VARS['user_id']);
		exit;
	}
	elseif($HTTP_POST_VARS['image_insert'])
	{
		...
		bx_db_insert($bx_db_table_banner_banners, "banner_id,user_id,banner_name,format,type_id,url,alt,active,views,clicks,permission,weight,weight_counter", "'', '".$HTTP_GET_VARS['user_id']."', '', '".$HTTP_GET_VARS['format']."', '|".$HTTP_POST_VARS['banner_type']."|', '".$url."', '".$HTTP_POST_VARS['alt']."', 'true','0','0', '', '".$HTTP_POST_VARS['banner_weight']."', '0'");
		SQL_CHECK(0,"SQL Error at ".__FILE__.":".(__LINE__-1));
		...
		$selectSQL = "select without_review from $bx_db_table_banner_users where user_id='".$HTTP_GET_VARS['user_id']."'";
		$select_client_query = bx_db_query($selectSQL);
		SQL_CHECK(0,"SQL Error at ".__FILE__.":".(__LINE__-1));
		$select_client_result = bx_db_fetch_array($select_client_query);
...
elseif ($HTTP_GET_VARS['format']=="remote")
{
    if (!$HTTP_POST_VARS['banner_id'] && $HTTP_POST_VARS['remote_insert'])
    {
        ...
        else
        {
            ...
            $selectSQL = "select * from $bx_db_table_banner_users where user_id='".$HTTP_GET_VARS['user_id']."'";
            $select_client_query = bx_db_query($selectSQL);
            SQL_CHECK(0,"SQL Error at ".__FILE__.":".(__LINE__-1));
            $select_client_result = bx_db_fetch_array($select_client_query);
            ...
            bx_db_insert($bx_db_table_banner_banners ,"banner_id,user_id,banner_name,format,type_id,url,alt,active,views,clicks,permission,weight,weight_counter", "'', '".$HTTP_GET_VARS['user_id']."', '".$HTTP_POST_VARS['banner_name']."', '".$HTTP_GET_VARS['format']."', '|".$HTTP_POST_VARS['banner_type']."|', '".$url."', '".$HTTP_POST_VARS['remote_banner_type']."', 'true','0','0', '".$permission."', '".$HTTP_POST_VARS['banner_weight']."', '0'");
    ...
    elseif ($HTTP_POST_VARS['banner_id'] && $HTTP_POST_VARS['remote_insert'])
    {
        ...
        else
        {
            ...
            refresh("banners.php?user_id=".$HTTP_SESSION_VARS['pbm_userid']);
            exit();	
        }
    } //end if
    else{}
...
elseif($HTTP_GET_VARS['format'] && $HTTP_POST_VARS['html_insert']=="yes" && !$HTTP_GET_VARS['banner_id']) 
{
	...
	if(!is_admin())
		$HTTP_POST_VARS['banner_weight'] = '1';
	bx_db_insert($bx_db_table_banner_banners ,"banner_id,user_id,banner_name,format,type_id,url,alt,active,views,clicks,permission,weight,weight_counter", "'', '".$HTTP_GET_VARS['user_id']."', '', '".$HTTP_GET_VARS['format']."', '|".$HTTP_POST_VARS['banner_type']."|', '".$url."', '".$HTTP_POST_VARS['alt']."', 'true','0','0', '".$permission."', '".$HTTP_POST_VARS['banner_weight']."', '0'");
