...
var v,for_nr,x,yyy_length,temp_error;
	...
	ban_zones = temp_radio.length / document.banner_form['banner_planning1'].length;
	...
	for (z_nr=1; z_nr<=ban_zones; z_nr++)
	{
		for (x=0; x<for_nr; x++)
		{
			...
			if (yyy[x].checked)
			{
				...
				if (((x+1)%3)==0)
				{
					temp_total = temp_total + (temp_radio[radio_sel])*1000;
				}else{
					if (temp_base[nr_sel] > temp_number[nr_sel].value)	
					{
						alert(temp_name[nr_sel]+'\n<?=TEXT_MINIMUM?>'+temp_base[nr_sel]);
						temp_error = 1;
					}else if (!isNaN(temp_base[nr_sel] && !isNaN(temp_number[nr_sel].value))){
						temp_total = temp_total + ((temp_radio[radio_sel])/temp_base[nr_sel])*temp_number[nr_sel].value*1000;
					}
				}
			}
		}
